...
function MechPartManager:partConfig(partType, itemDescriptor)
  ...
  if itemConfig then
    ...
    if type(mechPartConfig) == "table" and #mechPartConfig == 2 and mechPartConfig[1] == typeKey then
      if itemConfig.parameters.partConfig and type(itemConfig.parameters.partConfig) == "table" then
        defaultConfig = copy(self.partTypeConfigs[typeKey][mechPartConfig[2]]) or {}
        customConfig = copy(itemConfig.parameters.partConfig)
        return util.mergeTable(defaultConfig, customConfig)
      elseif self.partTypeConfigs[typeKey] then
      ...
      if self.partStatMap[partType] and thisPartConfig.stats then
        for stat, fMap in pairs(self.partStatMap[partType]) do
          for param, fName in pairs(fMap) do
            if not thisPartConfig.partParameters[param] then
